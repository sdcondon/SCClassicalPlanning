...
public static class ProblemInspector
{
    ...
    public static VariableSubstitution GetMappingFromSchema(Action action, IQueryable<Action> actionSchemas, out IEnumerable<Literal> extraPreconditionElements)
    {
        ...
        // Probably worth an investigation into performance here, but just returning the first hit is fine for now.
        var mappingFromSchema = MatchWithSchema(action, matchingSchema, new VariableSubstitution()).FirstOrDefault()
            ?? throw new ArgumentException($"No consistent mapping could be found from '{action.Identifier}' schema to action with precondition {action.Precondition} and effect {action.Effect}");
